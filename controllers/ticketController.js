...
🚨 CRITICAL EC261 ELIGIBILITY RULES (EVALUATE IN THIS EXACT ORDER):
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(`[Gemini API] Transient error: ${apiError.message}. Retrying in ${waitTime / 1000}s...`);
  ...
  // -------------------------------------------------------------------------
  // Post-processing: distance, claim value, doc checker, jurisdiction
  // -------------------------------------------------------------------------
  parsedJourneys.forEach(journey => {
    ...
    journey.routes.forEach(route => {
      ...
      route.legs.forEach(leg => {
        ...
        // --- 0. Fix AI-split flight numbers ---
        if (Array.isArray(leg.flightNumbers)) {
          let cleaned = leg.flightNumbers
            .map(fn => fn.replace(/[\s-]/g, '').trim())
            .filter(Boolean);
          ...
          for (let i = 0; i < cleaned.length; i++) {
            if (
              i < cleaned.length - 1 &&
              /^[A-Za-z0-9]{2,3}$/.test(cleaned[i]) &&
              /^\d{1,4}$/.test(cleaned[i + 1])
            ) {
        ...
        if (originPort && destPort) {
          ...
          const a =
            ...
            Math.cos(originPort.lat * Math.PI / 180) *
            Math.cos(destPort.lat  * Math.PI / 180) *
          ...
          const euCountries = [
            ...
            'slovakia','slovenia','spain','sweden','iceland','norway',
          ...
          if      (distance <= 1500)                          leg.ec261Leg.estimatedClaimValue = '€250';
          else if (isIntraEU || distance <= 3500)             leg.ec261Leg.estimatedClaimValue = '€400';
          else                                                leg.ec261Leg.estimatedClaimValue = '€600';
        } else {
        ...
        const opLimitRaw       = getJurisdictionLimit(opCountry);
        const opLimitFormatted = opLimitRaw !== 'N/A' ? `${opLimitRaw} years` : 'N/A';
        const displayOpCountry = leg.operatingAirlineCountry && leg.operatingAirlineCountry !== 'Unknown'
          ? leg.operatingAirlineCountry : 'Unknown HQ';

        const mktCountry        = (leg.marketingAirlineCountry || '').toLowerCase().trim();
        const mktLimitRaw       = getJurisdictionLimit(mktCountry);
        const mktLimitFormatted = mktLimitRaw !== 'N/A' ? `${mktLimitRaw} years` : 'N/A';
        const displayMktCountry = leg.marketingAirlineCountry && leg.marketingAirlineCountry !== 'Unknown'
          ? leg.marketingAirlineCountry : 'Unknown HQ';

        if (marketing === operating) {
          leg.claimDocuments = [
            { airline: marketing, role: '',        reqs: getAirlineReqs(marketing), hq: displayOpCountry, limit: opLimitFormatted },
          ];
        } else {
          leg.claimDocuments = [
            { airline: marketing, role: 'Booked',  reqs: getAirlineReqs(marketing), hq: displayMktCountry, limit: mktLimitFormatted },
            { airline: operating, role: 'Operated', reqs: getAirlineReqs(operating), hq: displayOpCountry,  limit: opLimitFormatted  },
          ];
        }

        // --- 3. Jurisdiction / expiration override ---
        if (leg.ec261Leg && leg.ec261Leg.claimExpiration) {
          const oCountry = (leg.originCountry      || '').toLowerCase().trim();
          const dCountry = (leg.destinationCountry || '').toLowerCase().trim();

          const oLimit   = getJurisdictionLimit(oCountry);
          const dLimit   = getJurisdictionLimit(dCountry);
          ...
          // Pick the best (highest numeric) jurisdiction
          let bestLimit       = 0;
          let bestCountryName = 'Unknown';

          const candidates = [
            { limit: oLimit,      name: leg.originCountry },
            { limit: dLimit,      name: leg.destinationCountry },
            { limit: opLimitRaw,  name: leg.operatingAirlineCountry },
            { limit: mktLimitRaw, name: leg.marketingAirlineCountry },
          ];

          for (const { limit, name } of candidates) {
            // Only numeric values participate in "best" selection
            const numeric = typeof limit === 'number' ? limit : null;
            if (numeric !== null && numeric > bestLimit) {
              bestLimit       = numeric;
          ...
          if (bestLimit > 0 && leg.date && leg.date !== 'Unknown') {
            ...
            if (!isNaN(flightDate.getTime())) {
              ...
              leg.ec261Leg.claimExpiration.isExpired      = new Date() > flightDate;
            }
          } else {
...
exports.checkFlightStatus = async (req, res, next) => {
  try {
    ...
    const filterByRoute = (originFilter, destFilter) =>
      data.flightStatuses.filter(f => {
        const oMatch = !originFilter || originFilter === 'Unknown' || f.departureAirportFsCode === originFilter.toUpperCase();
        const dMatch = !destFilter   || destFilter   === 'Unknown' || f.arrivalAirportFsCode   === destFilter.toUpperCase();
        const dtMatch = f.departureDate?.dateLocal?.startsWith(requestedDateStr);
        return oMatch && dMatch && dtMatch;
      });

    let exactMatches = filterByRoute(origin, destination);
    if (!exactMatches.length) exactMatches = filterByRoute(null, destination);
    if (!exactMatches.length) exactMatches = data.flightStatuses.filter(f => f.departureDate?.dateLocal?.startsWith(requestedDateStr));
